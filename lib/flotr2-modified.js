...
Series.prototype = {
  ...
  getRange: function () {
    ...
    for (i = 0; i < length; i++) {
	  ...
	  // modified stuff by me
	  
	  // series like stock prices can have more than one y value
	  yHigh = Math.max.apply(null, data[i].slice(1));
	  yLow  = Math.min.apply(null, data[i].slice(1));
      ...
      if (yHigh !== null) {
        if (yHigh > ymax) { ymax = yHigh; yused = true; }
      }
	  if (yLow !== null) {
        if (yLow < ymin) { ymin = yLow; yused = true; }
      }
	  // end modification
    }
